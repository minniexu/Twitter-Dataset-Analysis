...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 17 20:22:32 2016
...
#                            
    ...
    plt.title('num_tweet vs Prediction')
    plt.show()
    
    fig, ax = plt.subplots()
    ax.scatter(T2,pre)
    #ax.plot([min(T2), max(T2)], [min(T2), max(T2)], lw=4)
    ax.set_xlabel('num_retweets')
    ax.set_ylabel('prediction value')
    plt.title('num_retweets vs Prediction')
    plt.show()
    
    fig, ax = plt.subplots()
    ax.scatter(T3,pre)
    #ax.plot([min(T3), max(T3)], [min(T3), max(T3)], lw=4)
    ax.set_xlabel('max_follower')
    ax.set_ylabel('prediction value')
    plt.title('max_follower vs Prediction')
    plt.show()
